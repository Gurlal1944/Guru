...
me=LINE("EAV7XiaKsUkGoL195t52.TvdRmJaim08TzpRrtq9y8G.hmhPSwQ2lmBSukrLQouNupSi4WuAC5pALkc1X2OOzvM=")
...
apikey_com = "ub550eb323008334f2d64fad14d0f58f2"
coverId = me.getProfileDetail()["result"]["objectId"]
...
def waktu(secs):
    ...
    hours, mins = divmod(mins,60)
    days, hours = divmod(hours, 24)
    return '%02d Days %02d Hours %02d Minuts %02d Secons' % (days, hours, mins, secs)
def Run_Xp():
    print ("RESTART")
    backupData()
...
def RunTheRun(to, mid, firstmessage):
    try:
        ...
        text += mention+": "+datetime.strftime(timeNow,'')+" My location : LONDON: ""\n: ""\n\nTIME VPS : In "+hari+"\n\nCREATOR :- •●◉✿ Ⓖⓤⓡⓤ ✿◉●•\n\nDATE : "+datetime.strftime(timeNow,'%Y-%m-%d')+"\n\nTIME RUN : \n • "+bot+"\n\nMY ID LINE : IAMGURU12"
        me.sendMessage(to, text, {'MENTION': str('{"MENTIONEES":' + json.dumps(arr) + '}')}, 0)
    except Exception as error:
        me.sendMessage(to, "Error :\n" + str(error))
def SqL_R(text):
                ...
                if msg.contentType == 0:
                    if text is None:
                        ...
                        if PrankBotsData == Abouts["6"]:
                          if D in Owner or D in meM:
                            result = requests.get("http://NETFLIX.COM/")
                        ...
                        if PrankBotsData.startswith(Abouts["10"]):
                          if D in Owner or D in meM:
                            ...
                            me.sendMessage(R, "FRIEND ADDED")
                        if PrankBotsData == Abouts["11"]:
                          if D in Owner or D in meM:
                            me.sendMessage(R, "Guru's Bot Restarted...")
                            PrankBots["Conection"] = R
                            Run_Xp()
                        ...
                        if PrankBotsData == Abouts["14"]:
                          if D in Owner or D in meM:
                            ...
                            me.sendMessage(R, "━━━━━╦RUNTIME OF BOT╦━━━━━\n ┣━╦[ {}".format(str(runtime)))
                        ...
                        if PrankBotsData.startswith(Abouts["25"]):
                          if D in Owner or D in meM:
                            ...
                            with requests.session() as web:
                                ...
                                ret_ = "╭━━━━━[ Youtube links of your search ]━"
                                datas = []
                        ...
                        if PrankBotsData == Abouts["37"]:
                          if D in Owner or D in meM:
                            me.sendMessage(R, "━━━━══[Group Creator]══━━━━")
                            group = me.getGroup(R)
                        ...
                        if PrankBotsData == Abouts["45"]:
                          if D in Owner or D in meM:
                            if msg.toType == 2:
                                group = me.getGroup(R)
                                if group.preventedJoinByTicket == False:
                                    me.sendMessage(R, "Already opened")
                                else:
                                    group.preventedJoinByTicket = False
                                    me.updateGroup(group)
                                    me.sendMessage(R, "Successfully opened Qr")
                        if PrankBotsData == Abouts["46"]:
                          if D in Owner or D in meM:
                            if msg.toType == 2:
                                group = me.getGroup(R)
                                if group.preventedJoinByTicket == True:
                                    me.sendMessage(R, "already closed")
                                else:
                                    group.preventedJoinByTicket = True
                                    me.updateGroup(group)
                                    me.sendMessage(R, "Successfully closed Qr")
            ...
            if PrankBots["Unsend"] == True:
                ...
                if Text_in_Destroy in msg_dict:
                    ...
                    if "text" in msg_dict[Text_in_Destroy]:
                        StartTimer = Timer - msg_dict[Text_in_Destroy]["Time"]
                        ...
                        if Gr in Sid['Red']:
                            ...
                            if Name in Sid['Reason'][Gr]:
                                ...
                                if " " in Name:
                                    nick = Name.split(' ')
                                    if len(nick) == 2:
                                        me.sendMessage(Gr, "I see you" + " " + nick[0] + " " + "\ndear say hi . . .\n😀😀 ")
                                        me.sendImageWithURL(Gr, "" + Np)
                                    else:
                                        me.sendMessage(Gr, "Read by " + " " + nick[1] + " " + "\nHii dear.\n😀😀")
                                        me.sendImageWithURL(Gr, "/" + Np)
                                else:
                                    me.sendMessage(Gr, "hello " + " " + Name + " " + "\ni See you \n 😀😀")
                                    me.sendImageWithURL(Gr, "" + Np)
                        else:
                            pass
                else:
